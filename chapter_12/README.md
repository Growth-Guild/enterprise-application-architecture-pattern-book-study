...
## 구현 테이블 상속
* 구현 테이블 상속은 상속 계층에 포함된 각 구현 클래스마다 테이블 하나를 사용해 구성한다.

### 작동 원리
* 구현 테이블 상속은 계층의 각 구현 클래스당 데이터베이스 테이블 하나를 사용한다.
* 각 테이블에는 구현 클래스와 해당하는 모든 조상의 열이 포함되므로 상위 클래스의 모든 필드는 하위 클래스의 전체 테이블에 복제된다.
* 이 패턴을 사용할 때는 키가 해당 테이블은 물론이고 계층 전체 테이블에서 고유해야 한다는 점을 주의해야 한다.
  * 구현 클래스를 매핑하는 테이블 간에 키가 복제되면 특정 키 값 하나가 여러 행과 연결될 수 있다.
  * 따라서 전체 테이블에서 사용되는 키를 추적하는 키 할당 시스템이 필요하며, 데이터베이스에 있는 기본 키 고유성 메커니즘에 의존할 수는 없다.

### 사용 시점
* 상속을 매핑할 때는 구현 테이블 상속, 클래스 테이블 상속, 단일 테이블 상속 중 하나를 사용할 수 있다.
* 구현 테이블 상속의 장점
  * 각 테이블이 독립적이며 무의미한 필드가 없다.
  * 구현 매퍼에서 데이터를 읽을 때 조인을 할 필요가 없다.
  * 해당 클래스에 접근할 때만 각 테이블에 접근하며 접근 부하를 분산할 수 있다.
* 구현 테이블 상속의 단점
  * 기본 키를 처리하기 어려울 수 있다.
  * 추상 클래스에 대한 데이터베이스 관계를 강제할 수 없다.
  * 도메인 클래스의 필드를 계층에서 위아래로 이동할 경우 테이블 정의를 변경해야 한다.
  * 상위 클래스 필드가 전체 테이블에 복제되므로 상위 클래스 필드가 변경되면 이 필드가 있는 테이블 모두 변경해야 한다.
  * 상위 클래스에서 검색하려면 모든 테이블을 확인해야 하므로 데이터베이스에 여러 번 접근하거나 불편한 조인을 해야 한다.

## 상속 매퍼
* 상속 계층을 처리하는 데이터베이스 매퍼를 구성하는 구조를 상속 매퍼라고 한다.

### 작동 원리
* 계층과 함께 매퍼를 구성해 도메인 클래스별 매퍼를 통해 해당 도메인 클래스의 데이터를 저장하고 로드할 수 있다.
  * 이렇게 하면 매핑을 한 지점에서 변경할 수 있다.
* 이 체계는 계층 구조의 특정 부분에 필요한 정보를 저장하는 매퍼를 손쉽게 작성할 수 있다.

### 사용 시점
* 일반적 체계는 모든 상속 기반 데이터베이스 매핑에 적용할 수 있다.